...
def main():
    ...
    if not os.path.exists('config/articles.md'):
        logger.error("config/articles.md not found. Please create it with PDF URLs.")
        sys.exit(1)
    
    # Step 1: Download PDFs
    logger.info("=== Step 1: Downloading PDFs ===")
    downloader = PdfDownloader(output_dir="data/raw/pdfs", delay=2)
    download_results = downloader.download_from_file("config/articles.md")
    ...
    print(f"  📁 Output directory: data/raw/scraped_data/pdfs/")
